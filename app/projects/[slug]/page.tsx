...
import { ProjectJsonLd, BreadcrumbJsonLd } from "@/components/project-jsonld";
...
export function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  if (!project) return {};
  const title = `${project.title} | Asjid Siddique`;
  return {
    title,
...
export default function ProjectDetailPage({
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <Nav />
      <main id="main-content" className="mx-auto max-w-content px-6 py-16">
        <nav aria-label="Breadcrumb" className="mb-6 flex items-center gap-2 text-xs text-muted">
          ...
          <span>/</span>
          <Link href="/#projects" className="transition-colors hover:text-cyan">
            Projects
          </Link>
          <span>/</span>
        ...
        {project.metrics.length > 0 && (
          <dl className="mt-10 grid grid-cols-2 gap-6 border-y border-edge py-8 sm:grid-cols-4">
            {project.metrics.map((m) => (
              <div key={m.label}>
                <dd className={`data-figure text-2xl ${accent.text}`}>
